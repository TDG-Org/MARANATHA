...
export function createFramePacer(startTime = 0) {
  ...
  const measuredPeriod = () => {
    ...
    let min = Infinity;
    for (let i = 0; i < n; i++) if (periods[i] < min) min = periods[i];
  ...
  const pacer = {
    ...
    advance(now, requestedFps) {
      ...
      const period = measuredPeriod();
      const interval = snapToRefresh(fps, period, {
      ...
      if (period > 0) {
        ...
        // d-th refresh", so the deadline is re-anchored to the frame that just
        // happened. Accumulating instead (nextDue += interval) lets a period
        // measurement that is a hair short drift earlier and earlier until it
        // eventually admits a frame a whole refresh early — one uneven gap in
        // an even stream, which is the artefact this all exists to remove.
